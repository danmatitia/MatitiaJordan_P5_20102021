/** Recuperation des produits stockés dans le local storage */
let products = [];
let productInLocalStorage = JSON.parse(localStorage.getItem('product'));

/** Afficher les produits dans le panier  */

/** Sélection de  cartAndFormContainer */
...
productInLocalStorage[j] = newLocalStorage;
    ...
    /** avertir les modifications et mettre à jour les totaux */ 
...
function deleteArticle() {
    ...
    for (let k = 0; k < deleteItem.length; k++) { 
      ...
      /**  callback l'élément cliqué par le bouton supprimer */
...
/** Envoi du formulaire dans le server */
function postForm() {
    const order = document.getElementById('order');
    order.addEventListener('click', (event) => {
    event.preventDefault();

/** je mets les données du formulaire dans un objet   */
const contact = {
...
function controlName() {
    ...
    if (/^[^0-9_!¡?÷?¿/\\+=@#$%ˆ&*(){}|~<>;:[\]]{3,20}$/.test(validName)) {
        ...
        lastNameErrorMsg.innerText = "Merci de vérifier le nom, 3 caractères minimum et des lettres uniquement";
    }
}
...
/** valeur fomulaire et produit dans un objet */
const sendFormData = {
    contact,
    products,
}

/** Envoi du formulaire et du localstorage dans le server */
const options = {
method: 'POST',
